...
void khub(network *, int, int);//calculate S(lb),E(lb)
...
int main()
{
    ...
    ofstream outFilek;
    outFilek.open("i_k.txt");

    for (int i=0; i<N; i++)
        outFilek << i << "\t" << int(node[i].neighbor.size()) << endl;
    outFilek.close();
...
void khub(network *, int Nb, int Rb)
{

    char Filename[40];
    sprintf(Filename,"k_vs_khub_stage0_rb_%d.dat",Rb);
    ifstream inFile;
...
int readbox(network * node, int Rb)
{
