...
{
  int size, mr, nb, lb, mb;
  FILE *fp;
  size =  atoi(argv[2]);
  fp = fopen(argv[3], "w");
  mb = size;
  lb = 0;
  fprintf(fp, "%d\t%d\t%f\n", lb*2 + 1, mb, (double) mb/size);
  while ( mb>1 ){
    lb ++;
    mb = MEMB(argv[1], size, lb);
    fprintf(fp, "%d\t%d\t%f\n", lb*2 + 1, mb, (double) mb/size);
  }
  fclose(fp);
  return 0;
...
int  MEMB(char *fileName, int cluster_size, int l_b)
//int argc;
//char *argv[];
{
	int i,j,boxno,paint_box();
	char name[50];
	...
	//else
        //{
        //        printf("Usage: %s filename number_of_nodes r_B output_file_name\n",argv[0]);
        //        return(-1);
        //}
	...
	//{
		...
		*/
		if(new_size>1)
		{
			sprintf(name,"boxes_rb_%d.dat", l_b);
			sprintf(name, "boxes
			fp=fopen(name,"w");
